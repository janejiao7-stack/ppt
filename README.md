...
```bash
bash scripts/run_ppt_migration_dryrun.sh   --run-id trial-001   --source path/to/source.pptx   --theme path/to/theme.thmx
```
...
```bash
bash scripts/execute_theme_swap_trial.sh   --run-id trial-live   --source path/to/old.pptx   --template path/to/template.pptx
```
